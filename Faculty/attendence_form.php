	...
	<body>
		...
		<div id="main">
			<div style="overflow:auto;">
				<form id="form" action="<?php echo $_SERVER['PHP_SELF'];?>" method="POST" class="form-horizontal ac-custom ac-checkbox ac-cross">
							...
							if($batch=='tut-t1' || $batch=='lab-t1')
							{
								...
								if($stu>0)
								{
									...
									echo"<tr>";
										...
										echo "<td>"."<input type='checkbox' name='".$row['rollnumber']."'>"."<label for='cb1'>"."PRESENT"."</label>"."</td>";
									echo "</tr>";
								}
							}
					...
					<button class="btn btn-default" id="button" type="submit" name="submit_attendence" value="SUBMIT">Submit</button>
				</form>
			</div>
		</div>
		...
		<?php
			...
			if(isset($_POST['submit_attendence']))
			{
				...
				$rollnumber_select_query="SELECT rollnumber FROM `attendence-semester-".$semester."`";
				$rollnumber_select_query_run=mysqli_query($connection,$rollnumber_select_query);
			

				while($row=mysqli_fetch_assoc($rollnumber_select_query_run))
				{
				
					if($batch=="Lecture")
					{
						/*--------------------------------------------------------------
						*	 CHECKING IF PERSON IS PRESENT AND DO REQUIRED MANUPULATION
						*--------------------------------------------------------------
						*/		
						take_attendence($row['rollnumber'],$batch);
						update_overall_attendence($row['rollnumber']);
							
					}
					else if($batch=='tut-t1' || $batch=='lab-t1')
					{
						
						$query_match_student_batch="SELECT * FROM `student-detail` WHERE `rollnumber`='".$row['rollnumber']."' AND `group-batch`='t1'";
						$query_run_match_student_batch=mysqli_query($connection,$query_match_student_batch);
						$num_rows=mysqli_num_rows($query_run_match_student_batch);
						
						if($num_rows>0)
						{
							$value1=substr($batch,0,3);
							take_attendence($row['rollnumber'],$value1);
						}
						update_overall_attendence($row['rollnumber']);
					}
					else if($batch=='tut-t2' || $batch=='lab-t2')
					{
						...
						$query_match_student_batch="SELECT * FROM `student-detail` WHERE `rollnumber`='".$row['rollnumber']."' AND `group-batch`='t2'";
						$query_run_match_student_batch=mysqli_query($connection,$query_match_student_batch);
						$num_rows=mysqli_num_rows($query_run_match_student_batch);
				...
				$( function() {
					...
					$( "#dialog-message" ).dialog({
						...
						buttons: {
							Ok: function() {
								window.location="take_attendence_temp.php";
							}
						}
					});
				});
		...
		*/						
			...
			function take_attendence($rollnumber,$batch1)
			{
				 global $subject_code;
				 global $batch;
				 global $connection;
